...
| R3b | 6.8kΩ | +15V → Q1 base (upper bias) | Upper leg of the BD139 base voltage divider. With R4, sets the open-circuit base voltage to ~1.92V. First-order that puts the emitter at ~1.22V (Ic ≈ 18mA); in the verified SPICE model, base-current loading and the real Vbe settle it to Ve ≈ 1.09V (Ic ≈ 16mA). Both are well inside the 1.0–1.4V / 10–26mA pass band — measure Ve, don't trust the divider arithmetic. |
| R4 | 1kΩ | Q1 base → GND (lower bias) | Lower leg of the bias divider. Divider current = 1.92mA; base current at hFE=100 is 0.18mA (ratio ≈ 11×). However, BD139 hFE can be as low as 40, at which base current rises to ~0.45mA and the ratio drops to ~4× — the divider alone does not fully swamp hFE variation. R5 emitter degeneration is what actually holds the operating point stable across units. Verify bias by measuring Q1 emitter voltage (verified sim ≈1.09V; anything in 1.0–1.4V is good), not by trusting the divider calculation. |
| R5 | 68Ω | Q1 emitter → GND | Emitter degeneration resistor. Sets quiescent current (Ic ≈ Ve/R5; first-order 1.22V/68Ω ≈ 18mA, verified-sim 1.09V/68Ω ≈ 16mA) and provides thermal stability — if Q1 heats up and hFE rises, the increased emitter voltage reduces Vbe, self-limiting the current. This is the primary bias-stabilizing element; the divider (R3b/R4) provides the initial setpoint. Without this resistor, the transistor will thermally run away. |
...
| Rf | 100kΩ | U2 feedback — output → (−) pin | Upper leg of the non-inverting feedback divider. 100kΩ chosen to keep thermal noise contribution below the OPA2134's own input noise. **Cf (22pF C0G/NP0 across Rf) is a required BOM part — see the Cf row below.** |
| Cf | 22pF C0G/NP0 | U2 feedback compensation — `u2_out` → `u2_inv` (parallel with Rf) | At 214× gain, stray capacitance at U2's inverting input forms a pole with Rf that causes HF gain peaking or oscillation on perfboard. Cf rolls the loop off cleanly: 100kΩ + 22pF → ~72kHz corner, above the audio band, no audible high-end loss. **Required BOM part — do not skip.** Use C0G/NP0 ceramic (stable dielectric; this is a feedback path). |
...
|---|---|---|---|---|
| C1 | 1µF / 63V | WIMA MKS2 film | Dwell-wiper → Q1 base coupling (named **`C_drive`** in the SPICE netlist) | Blocks any DC offset from the input buffer output before the driver stage. Prevents DC from biasing the Dwell pot and reaching Q1's base. At 1µF with the ~10kΩ driver input impedance, the high-pass corner is ~16Hz — well below guitar fundamentals, so no bass rolloff in the reverb drive. (Schematic/BOM designator C1 = netlist instance `C_drive`; same physical part.) |
...
| C_bright | 47pF | Silver mica | Across the FULL RV2 Mix pot — lug 1 (CCW, `mix_dry`) to lug 3 (CW, `mix_wet`), **not** lug-to-wiper | Bright cap — maintains high-frequency content in the reverb tail at low mix settings. The Mix pot acts as a voltage divider at audio frequencies; at low settings it attenuates high frequencies more than low. The 47pF cap bypasses the pot for HF, keeping the reverb "glassy." Silver mica is specified for its extreme stability (±1% over temperature) and lowest possible distortion at high frequencies. Do not use ceramic disc — it will add HF distortion audible in reverb tails. |

### Signal Path — Electrolytic
| Ref | Value | Type | Location | Why |
|---|---|---|---|---|
| C2 | 100µF / 25V | Nichicon UKW (low-ESR audio grade), UKW1E101MED | Q1 emitter bypass — **stays 100µF; do not substitute the 25µF stock used for C13/C14** (C2 sets the driver's bass corner; it is a signal-path part that only used to share a P/N with the PSU caps — see issue #94) | Bypasses R5 at audio frequencies, allowing full AC gain from the driver stage. Without this cap, R5 provides heavy degeneration at audio frequencies and the spring tank is underdriven. Low-ESR audio grade is specified because a high-ESR cap will not effectively bypass R5 at high audio frequencies — the driver's HF response will roll off early, losing the "air" in the reverb sound. 25V voltage rating is appropriate: C2 sits at Q1's emitter (~1.1V), not on a supply rail, and never sees unregulated transients. |
...
| Ref | Value | Type | Location | Why |
|---|---|---|---|---|
| C11–C12 | 1000µF / **50V** | Nichicon UKW (low-ESR) | Main filter after bridge rectifier | Primary AC ripple filtering. *(Downsized from 2200µF — see issue #95.)* At the ~40mA worst-case + rail load, 1000µF gives ~350mVpp bus ripple, which the regulators' ~68dB 120Hz rejection turns into well under 1mV on the rails; the low-mains (108VAC) unregulated trough stays ~0.75V above the regulators' 17V dropout floor. 2200µF was sized for amplifier-class loads this unit doesn't have; 1000µF halves can size, power-on inrush, and bleed-down time. **50V rating retained** — the unregulated rail sits at ~21V, so 50V gives 2.4× margin; capacitor lifespan increases significantly when operated well below rated voltage, and the cost difference is negligible. |
| C13–C14 | 75µF (3 × 25µF in parallel) / ≥25V | 25µF aluminum electrolytic — **off-BOM, sourced separately** (25µF is off the standard E-series; to be procured) | LM7815/7915 output | Regulator output stability cap — **not** ripple filtering (that happens at C11/C12). The LM78xx/79xx series requires an output cap to prevent oscillation; the LM7915 datasheet floor is ≥25µF aluminum, so 75µF clears it 3× over and still provides local energy storage for the op-amp stages. *(Changed from a single 100µF to 3 × 25µF parallel per rail — see issue #94.)* **Parallel only — never series** (series would give 8.3µF, below the stability floor; balancing resistors are only a series-stack concern, parallel needs none). Keep the C17/C18 film bypasses in place. |
...
| Power LED + bezel | 5mm high-efficiency LED + panel-mount bezel, 10kΩ current-limiting resistor from +15V rail | Visual confirmation that the unit is powered. Wire from +15V rail through 10kΩ resistor to LED anode, cathode to GND. At 10kΩ, drive current is ~1.2mA — subtle, not a beacon, which is appropriate for rack use. **Use a high-efficiency 5mm LED** — the BOM specs Kingbright **WP7113ID** (red) with a panel bezel (Mouser 593-MBC255-RD or equivalent); standard commodity LEDs are optimized for 20mA and will be dim at this current. **The 10kΩ current-limit resistor comes from the existing qty-5 10kΩ stock (same Yageo MFR-25FBF52-10K line that supplies Rdry + spares) — no separate resistor order needed.** Adjust value to taste: 4.7kΩ ≈ 2.5mA for brighter indication. |
...
| Tank grommets | 4× Shore 30A silicone rubber grommets, ~6mm ID, M3-compatible — e.g. McMaster-Carr 9600K11 or equivalent | The spring tank must be mechanically decoupled from the chassis. Any vibration transmitted through the chassis (footsteps, rack fans, amp vibration) will be picked up by the springs as noise. Soft grommets prevent this. Shore 30A (30–40) is essential — harder rubber transmits more vibration. **Mouser does not stock this grommet — source from McMaster-Carr or a hardware supplier (see mouser-bom.csv note).** |
