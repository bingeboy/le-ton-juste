...
7. **Unused OPA2134 section:** U3 occupies the first half of the second OPA2134PA package; the second half is unused. A floating spare op-amp section can oscillate internally and couple noise through the shared supply pins into U3. Tie the spare section's inputs to a defined level: connect the unused (−) input to GND and the unused (+) input to GND (both inputs to GND, leave the output floating). This takes two resistor-leg connections and costs nothing.
