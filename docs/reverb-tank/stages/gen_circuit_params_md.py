...
RESISTORS = [
    ...
    ("Rdry", ohm(P.RDRY), "Dry-path mixing resistor into Mix node", "`u1_buf` → `mix_top`"),
    ("Rwet", ohm(P.RWET), "Wet-path tie from Tone wiper to Mix node (≈0Ω model of direct wire)", "`rv3_wiper` → `mix_top`"),
    ...
    ("RV2a", ohm(P.RV2A), "Mix pot upper half (Mix top → wiper)", "`mix_top` → `mix_node`"),
    ("RV2b", ohm(P.RV2B), "Mix pot lower half (wiper → GND)", "`mix_node` → `0`"),
...
CAPACITORS = [
    ...
    ("C_bright", farad(P.C_BRIGHT), "Bright cap across Mix pot (HF presence at low mix)", "`mix_top` → `mix_node`"),
