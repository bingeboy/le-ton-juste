...
Delta from Stage 4 (stage_04_input_protect.asc):
  ...
  - ADD the full PSU that PRODUCES the +15V / -15V rails:
        ...
        Bulk filter + bleed (one set per rail):
            C11  pos_rect 0  1000u      ; +ve bulk filter
            R_bleed1 pos_rect 0  10k    ; bleed
            C12  neg_rect 0  1000u      ; -ve bulk filter
        ...
        Regulator output caps + HF bypass on the regulator output pin:
            C13  reg_pos 0  75u       ; 3x 25u in parallel
            C17  reg_pos 0  100n
            C14  reg_neg 0  75u       ; 3x 25u in parallel
            C18  reg_neg 0  100n
...
def build(active_analysis="op"):
    ...
    b.text(16, 8, "Stage 4 signal path UNCHANGED, now fed from the real PSU. T1 15-0-15VAC (two SINE(0 21.2 60), center tap=GND) -> BR1 (4x 1N4007) -> C11/C12 1000u bulk + 10k bleed -> U4 LM7815 / U5 LM7915 -> C13/C14 75u (3x 25u parallel) + C17/C18 100n -> F2/F3 polyfuses (0.5ohm) on DC rails -> +/-15V rails. Connectivity by net labels (FLAG at each pin).", 2)
