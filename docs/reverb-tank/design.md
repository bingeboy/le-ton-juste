...
> **Authority scope (single source of truth).** This is the **high-level architecture overview** — concept, topology, signal flow. It is **not** authoritative for any number. Figures quoted here (gain, corner frequencies, VA rating) are descriptive nominals. For authoritative values: canonical parameter table → [circuit-params.md](./circuit-params.md); component values → [`stages/stage_06_full.net`](./stages/stage_06_full.net); pass criteria → [`stages/test-assertions.md`](./stages/test-assertions.md); parts/PNs → [`mouser-bom.csv`](./mouser-bom.csv); rationale → [`parts-spec.md`](./parts-spec.md); bench procedures → [`stages/builder-guide.md`](./stages/builder-guide.md). If a number here disagrees with those, **they win.**
