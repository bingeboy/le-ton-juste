...
3. **Time effects after preamp.** QuadraVerb and reverb tank sit after the Alembic so reverb tails aren't colored by the preamp's EQ/gain stage. The QuadraVerb runs in a parallel loop via the **Lehle Parallel M** — the dry signal never touches digital conversion. The Ghost Spring stays in series after the Lehle (fully analog, its own Mix pot handles blend). QuadraVerb reverb programs are retired; it handles delay and modulation only.
